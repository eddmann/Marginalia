...
interface CopyURIRequest {
  uri: string;
  dst: string;
}

interface CopyURIResponse {
  success: boolean;
  error?: string;
}

interface SetSystemUIVisibilityRequest {
  visible: boolean;
  darkMode: boolean;
}

interface SetSystemUIVisibilityResponse {
  success: boolean;
  error?: string;
}

interface GetStatusBarHeightResponse {
  height: number;
  error?: string;
}

interface GetSystemFontsListResponse {
  fonts: Record<string, string>; // { fontName: fontFamily }
  error?: string;
}

interface InterceptKeysRequest {
  volumeKeys?: boolean;
  backKey?: boolean;
}

interface LockScreenRequest {
  orientation: 'portrait' | 'landscape' | 'auto';
}

interface GetSystemColorSchemeResponse {
  colorScheme: 'light' | 'dark';
  error?: string;
}

interface GetSafeAreaInsetsResponse {
  top: number;
  right: number;
  bottom: number;
  left: number;
...
interface GetStorefrontRegionCodeResponse {
  regionCode?: string;
  error?: string;
}
...
export async function interceptKeys(request: InterceptKeysRequest): Promise<void> {
  try {
    ...
    // native-bridge plugin not available
  }
}
