...
function getFoliateDataPath(dataDir: string, identifier: string): string {
  return `${dataDir}/com.github.johnfactotum.Foliate/${encodeURIComponent(identifier)}.json`;
}

/** Map a Foliate color string to a highlight style and color. */
function mapFoliateColor(color: string | undefined): {
...
function convertFoliateAnnotation(hash: string, annotation: FoliateAnnotation): BookNote {
...
function convertFoliateBookmark(hash: string, cfi: string): BookNote {
  const now = Date.now();
  return {
...
function convertFoliateData(hash: string, data: FoliateData): Partial<BookConfig> {
...
function parseFoliateData(json: string): FoliateData | null {
  try {
