...
declare global {
  interface Window {
    __MARGINALIA_IS_APPIMAGE?: boolean;
...
const nativeFileSystem: FileSystem = {
...
const DIST_CHANNEL = (process.env['NEXT_PUBLIC_DIST_CHANNEL'] || 'marginalia') as DistChannel;
...
export class NativeAppService extends BaseAppService {
  ...
  override isAppImage = Boolean(window.__MARGINALIA_IS_APPIMAGE);
  ...
  async saveFile(
    ...
    try {
      ...
      if (this.isIOSApp && options?.filePath) {
        // shareFile stubbed out (sharekit plugin removed)
